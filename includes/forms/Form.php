...
abstract class Form
{
    ...
    public function __construct($formId, $opciones = array())
    {
        ...
        /* Introduce las opciones para validar formularios con JS */ 
        if($opciones[0] !=  NULL){
            $this->validacion = "return ";
            $this->validacion .= $opciones[0]; 
        }
        if(isset($opciones[1]) && $opciones[1] != NULL){
            $this->action .= $opciones[1];
        }
        if(isset($opciones[2]) && $opciones[2] != NULL){
            $this->encType = $opciones[2]; 
        }
    }
    ...
    private function generaListaErrores($errores)
    {
        ...
        if (  $numErrores == 1 ) {
            $html .= "<ul><li class = 'error_form'>".$errores[0]."</li></ul>";
