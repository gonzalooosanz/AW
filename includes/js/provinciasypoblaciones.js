...
function toNameProvincia(idProvincia, pSelect) {
		 let i = 0; 
		 let encontrado = false; 
		 for(i = 0; i < pSelect.provinces.length && !encontrado; i++){
			 if(pSelect.provinces[i].id == idProvincia){
				 encontrado = true; 
			 }
		 }
		 return pSelect.provinces[i].nm + " "; 	 
	}
function toNameCiudad(idCiudad, pSelect){
	let i = 0; 
	 let encontrado = false; 
	 for(i = 0; i < pSelect.municipes.length && !encontrado; i++){
		 if(pSelect.municipes[i].id == idCiudad){
			 encontrado = true; 
		 }
	 }
	 return pSelect.municipes[i].nm + " "; 
}
...
;(function(root, factory) {
	...
	function create(provincesElement, municipesElement) {
		if(provincesElement != undefined && municipesElement != undefined){
		var self = this;
		...
		provincesElement.addEventListener('change', function () {
			...
			self.municipes.filter(function (mun) {
				return mun.id.toString().startsWith(province);
